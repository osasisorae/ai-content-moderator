...
Settings.llm = LlamaOpenAI(model="gpt-4.1-mini", temperature=0.1)
...
# Lazy-initialized index and query engine (deferred to avoid module-load-time API calls)
_index = None
_query_engine = None


def _get_query_engine():
    """Build the vector index and query engine on first use."""
    global _index, _query_engine
    if _query_engine is None:
        _index = VectorStoreIndex.from_documents(POLICY_DOCUMENTS)
